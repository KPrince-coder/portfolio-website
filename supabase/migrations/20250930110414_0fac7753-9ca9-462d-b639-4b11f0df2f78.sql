...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema='public' AND table_name='projects' AND column_name='description') THEN
    ALTER TABLE public.projects ADD COLUMN description JSONB NOT NULL DEFAULT '{}';
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema='public' AND table_name='projects' AND column_name='excerpt') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema='public' AND table_name='projects' AND column_name='duration') THEN
    ALTER TABLE public.projects ADD COLUMN duration INTEGER;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema='public' AND table_name='projects' AND column_name='technologies') THEN
    ALTER TABLE public.projects ADD COLUMN technologies TEXT[] DEFAULT '{}';
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema='public' AND table_name='projects' AND column_name='assets') THEN
    ALTER TABLE public.projects ADD COLUMN assets TEXT[] DEFAULT '{}';
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema='public' AND table_name='projects' AND column_name='seo_title') THEN
    ALTER TABLE public.projects ADD COLUMN seo_title TEXT;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema='public' AND table_name='projects' AND column_name='seo_description') THEN
    ALTER TABLE public.projects ADD COLUMN seo_description TEXT;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema='public' AND table_name='projects' AND column_name='author_id') THEN
    ALTER TABLE public.projects ADD COLUMN author_id UUID;
  END IF;
END $$;
...
CREATE POLICY "Public can view analytics for published projects" ON public.project_analytics
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM public.projects
      WHERE projects.id = project_analytics.project_id AND projects.status = 'published'
    )
  );

CREATE POLICY "Authenticated users can manage project analytics" ON public.project_analytics
  FOR ALL USING (auth.uid() IS NOT NULL);

-- RLS Policies for project_assets
CREATE POLICY "Public read assets for published projects" ON public.project_assets
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM public.projects
      WHERE projects.id = project_assets.project_id AND projects.status = 'published'
    )
  );

CREATE POLICY "Authenticated users can manage project assets" ON public.project_assets
  FOR ALL USING (auth.uid() IS NOT NULL);